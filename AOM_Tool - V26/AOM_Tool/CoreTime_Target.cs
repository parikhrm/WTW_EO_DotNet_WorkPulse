    ...
    class CoreTime_Target
    {
        ...
        public void coretime_roster_final_verification(DataTable dta, string associatename, DateTime dateroster)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Clear();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select distinct (convert(decimal(18,2),b.Contract_Hours) * (case when a.NoOfLeaves = 0 then 1 else a.NoOfLeaves end)) * (convert(decimal(18,2),[CoreTime%])/100) as CoreTime_Target from dbo.vw_roster_dotnet a inner join dbo.tbl_emp_details b on a.EmpID = b.[EmpId - New] where a.DateRoster = convert(date,@dateroster) and a.EmpName = @associatename";
                cmd.Parameters.AddWithValue("@dateroster", dateroster);
